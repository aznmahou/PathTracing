...
struct MixtureBSDF : BSDF {
    ...
    float pdf(const SurfaceInteraction& i) const override {
		...
		//diffuse pdf
		float pdfDiffuse = glm::max(0.0f,Warp::squareToCosineHemispherePdf(i.wi));
		...
		float pdfPhong = glm::max(0.0f, Warp::squareToPhongLobePdf(n, rotate));


		pdf = specularSamplingWeight * pdfPhong + (1.0f - specularSamplingWeight)*pdfDiffuse;
		return pdf;
    }

    v3f sample(SurfaceInteraction& i, const v2f& _sample, float* pdF) const override {
		v3f val(0.f);
		float probRR = _sample.x;
		//if less than specularSamplingWeight do phong else diffuse
		if (probRR < specularSamplingWeight) {
			probRR = _sample.x/specularSamplingWeight;

			float n = exponent->eval(worldData, i);
			v3f reflectDir = reflect(i.wo);
			v3f sampleDir = Warp::squareToPhongLobe(v2f(probRR,_sample.y), n);
		...
		else {//we do diffuse sampling
			probRR = (_sample.x- specularSamplingWeight)/(1.0f - specularSamplingWeight);

			i.wi = Warp::squareToCosineHemisphere(v2f(probRR,_sample.y));
