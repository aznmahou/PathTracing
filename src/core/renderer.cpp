...
void Renderer::render() {
	if (realTime) {
		...
		Sampler samplerX(260568705);
		Sampler samplerY(260568705);
		float scale = tan((deg2rad*scene.config.camera.fov) / 2.f);
		float aspectRatio = ((float)width / (float)height);


		for (int j = 0; j < width; j++) {
			for (int i = 0; i < height; i++) {
				glm::fvec3 colours = glm::fvec3(0.0, 0.0, 0.0);
				for (int q = 0; q < scene.config.spp; q++) {
					float randomerX = samplerX.next();
					float randomerY = samplerY.next();
					float x = (2.f*((float)j + randomerX) / ((float)width) - 1.f)*aspectRatio*scale;
					float y = (1.f - (2.f*((float)i + randomerY) / (float)height))*scale;
					glm::fvec4 pixel4 = glm::fvec4(x, y, -1.0f, 1.0f);
					glm::fvec4 dirWorld = operator*(pixel4, InverseView);
					glm::fvec3 dirWorld3 = glm::fvec3(dirWorld.x, dirWorld.y, dirWorld.z);
					Ray ray(scene.config.camera.o, dirWorld3);
					glm::fvec3 colors = integrator->render(ray, samplerX);
					colours += colors;
				}
				integrator->rgb->data[width*i + j] = colours / (float)scene.config.spp;
			}
			//cout << j << endl;
		}
	}
